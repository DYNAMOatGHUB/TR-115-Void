...
def generate_executive_summary(totals: dict, by_type: dict, supplier: str = None) -> str:
    ...
    context = f"""
Total emissions: {totals.get('total_kg', 0):.2f} kg CO₂e ({totals.get('total_tonnes', 0):.4f} tonnes CO₂e)
Scope 1 (direct): {totals.get('scope1_kg', 0):.2f} kg CO₂e
Scope 2 (energy): {totals.get('scope2_kg', 0):.2f} kg CO₂e
Scope 3 (supply chain): {totals.get('scope3_kg', 0):.2f} kg CO₂e
...
"""
    try:
        ...
        return (
            ...
            f"emissions of {total_kg:.2f} kg CO₂e. "
            f"Scope 1 direct emissions account for {totals.get('scope1_kg', 0):.2f} kg CO₂e, "
            f"Scope 2 energy-related emissions for {totals.get('scope2_kg', 0):.2f} kg CO₂e, "
            f"and Scope 3 supply chain emissions for {totals.get('scope3_kg', 0):.2f} kg CO₂e. "
...
def generate_markdown_report(pipeline_output: dict) -> str:
    ...
    lines.append(f"|--------|-------|")
    lines.append(f"| **Total CO₂e** | **{totals.get('total_kg', 0):.2f} kg** |")
    lines.append(f"| Total CO₂e (tonnes) | {totals.get('total_tonnes', 0):.4f} t |")
    ...
    lines.append("")
    lines.append("| Activity | CO₂e (kg) | % of Total |")
    lines.append("|----------|-----------|------------|")
    ...
    lines.append("")
    lines.append("| Description | Scope | CO₂e (kg) | Factor Source | Confidence |")
    lines.append("|-------------|-------|-----------|---------------|------------|")
    for r in results:
    ...
    if analyst.get("items_skipped", 0) > 0:
        ...
        for r in analyst.get("review_needed", []):
            lines.append(f"- **{r.get('description', 'Unknown')}**: {r.get('calculation_note', 'No factor found')}")
        lines.append("")
    ...
    lines.append("")
    lines.append(f"**Estimated total potential savings: {recommender.get('total_potential_savings_kg', 0):.2f} kg CO₂e**")
    lines.append("")
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        lines.append(f"**Priority Score:** {rec.get('priority_score', 0)}/10 | **Effort:** {effort.title()} | **Timeframe:** {timeframe.title()}")
        lines.append("")
        lines.append(rec.get('description', ''))
        lines.append("")
        lines.append(f"- Estimated reduction: **{pct}%** of {rec.get('target_activity', 'related')} emissions")
        lines.append(f"- Potential savings: **{savings:.2f} kg CO₂e**")
        lines.append("")
    ...
    lines.append("- **Formula:** CO₂e = Activity Data × Emission Factor")
